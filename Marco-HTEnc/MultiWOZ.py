    ...
    for num_dial, dialog_info in enumerate(source):
        ...
        for turn_num, turn in enumerate(dialog):
            ...
            if len(hist) == 0:
                ...
                hier_tokens = [2*turn_num]*len(tokens)
                ...
                hier_tokens = hier_hist + [2*turn_num] * (len(tokens) + 1)
                tokens = hist + [Constants.SEP_WORD] + tokens                

            # print(len(tokens), len(hier_tokens))
            assert len(tokens) == len(hier_tokens)
            
            if len(tokens) > max_seq_length:
                tokens = tokens[-max_seq_length:]
                hier_tokens = hier_tokens[-max_seq_length:]
            ...
            # ==================
            hist_tokens = tokens #.copy()
            tokens += source    
            # ==================
            hier_hist_temp = hier_tokens #.copy()
            hier_tokens += [2*turn_num+1]*len(source)
            assert len(tokens) == len(hier_tokens)
            # ==================
            if len(tokens) > (max_seq_length * 2):
                tokens = tokens[-(max_seq_length * 2):]
                hier_tokens = hier_tokens[-(max_seq_length * 2):]
...
#             print(hier_tokens)
            ...
            utt_loc = hier_tokens + [max(hier_tokens) + 1] * (max_seq_length * 2 - len(hier_tokens))
            MinH = min(utt_loc)
            utt_loc = [x - MinH for x in utt_loc]
            ...
            if turn_num == 0:
                ...
                hier_hist = hier_hist_temp + [2*turn_num + 1]*(len(sys) + 1)
            else:
                ...
                hier_hist = hier_hist + [hier_hist_temp[0]] + hier_hist_temp + [2*turn_num + 1]*(len(sys) + 1)
    ...
    all_hier_masks = torch.tensor([f[12] for f in examples], dtype=torch.float32)
    ...
    return all_input_ids, action_masks, \
           ...
           bert_act_seq, action_inp_ids, action_out_ids, labels, act_input_mask, resp_input_mask, all_hier_masks, all_files
