...
class RespGenerator(nn.Module):
    ...
    def __init__(self, vocab_size, act_vocab_size,
                 d_word_vec, nlayers_e, nlayers_d, d_model, n_head,
                 act_dim, dropout=0.1):
        ...
        self.nlayers_e = nlayers_e
        self.nlayers_d = nlayers_d
        ...
        self.enc_layer_stack = nn.ModuleList([
            EncoderLayer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
            for _ in range(nlayers_e)])

        self.layer_stack = nn.ModuleList([
            DecoderLayer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
            for _ in range(nlayers_d)])
    ...
    def resp_forward(self, tgt_seq, src_seq, act_vecs, act_mask, input_mask, utt_loc):
        # -- Encode source
        non_pad_mask = ~input_mask
        non_pad_mask = non_pad_mask.unsqueeze(-1).float()
        
        pe_utt_loc, enc_mask_utt, enc_mask_ct, dec_enc_attn_mask = _HIER_masks(tgt_seq, src_seq, input_mask, utt_loc)

        # Positional Embedding for Utterance Encoder
        enc_inp = self.word_emb(src_seq) + self.post_word_emb.forward_by_index(pe_utt_loc)# self.post_word_emb(src_seq)

        for i, layer in enumerate(self.enc_layer_stack):
            if i == self.nlayers_e//2:
                # Positional Embedding for Context Encoder
                enc_inp = enc_inp + self.post_word_emb(enc_inp)
            # enc_inp, _ = layer(enc_inp, non_pad_mask, enc_mask_utt)
            if i < self.nlayers_e//2:
                enc_inp, _ = layer(enc_inp, non_pad_mask, enc_mask_utt)
            else:
                enc_inp, _ = layer(enc_inp, non_pad_mask, enc_mask_ct)
...
#         raise Exception("Good Enough!")
        return logits

    def act_forward(self, tgt_seq, src_seq, bs, input_mask, utt_loc):
        # -- Encode source
...
#             enc_inp, _ = layer(enc_inp, non_pad_mask, enc_mask_utt)
    ...
    def resp_translate_batch(self, bs, act_vecs, act_mask, input_mask, src_seq, utt_loc, n_bm, max_token_seq_len=30,
                             gram_num=-1):
        """ Translation work in one batch """
        device = src_seq.device

        def collate_active_info(bs, act_vecs, act_mask, input_mask, src_seq, utt_loc, inst_idx_to_position_map,
            ...
            utt_loc = collect_active_part(utt_loc, active_inst_idx, n_prev_active_inst, n_bm)

            # active_template_output = collect_active_part(template_output, active_inst_idx, n_prev_active_inst, n_bm)

            active_inst_idx_to_position_map = get_inst_idx_to_tensor_position_map(active_inst_idx_list)

            return bs, act_vecs, act_mask, input_mask, active_src_seq, utt_loc, active_inst_idx_to_position_map
        ...
        def beam_decode_step(bs, inst_dec_beams, len_dec_seq, active_inst_idx_list, act_vecs, act_mask, input_mask,
            ...
            logits = self.resp_forward(dec_partial_seq, src_seq, act_vecs, act_mask, input_mask, utt_loc)[:, -1, :] / Constants.T
        ...
        with torch.no_grad():
            ...
            utt_loc = utt_loc.repeat(1, n_bm).view(n_inst * n_bm, -1)
            ...
            for len_dec_seq in range(1, max_token_seq_len + 1):
                active_inst_idx_list = beam_decode_step(bs, inst_dec_beams, len_dec_seq, active_inst_idx_list,
                                                        act_vecs, act_mask, input_mask, src_seq, utt_loc,
                                                        inst_idx_to_position_map, n_bm)

                if not active_inst_idx_list:
                    break  # all instances have finished their path to <EOS>

                bs, act_vecs, act_mask, input_mask, src_seq, utt_loc, inst_idx_to_position_map = collate_active_info(
                    bs, act_vecs, act_mask, input_mask, src_seq, utt_loc, inst_idx_to_position_map, active_inst_idx_list)
    ...
    def act_translate_batch(self, input_mask, bs, src_seq, utt_loc, n_bm, max_token_seq_len=30):
        """ Translation work in one batch """
        device = src_seq.device

        def collate_active_info(input_mask, bs, src_seq, utt_loc, inst_idx_to_position_map, active_inst_idx_list):
            # Sentences which are still active are collected,
            # so the decoder will not run on completed sentences.
            n_prev_active_inst = len(inst_idx_to_position_map)
            active_inst_idx = [inst_idx_to_position_map[k] for k in active_inst_idx_list]
            ...
            input_mask = collect_active_part(input_mask, active_inst_idx, n_prev_active_inst, n_bm)
            active_utt_loc = collect_active_part(utt_loc, active_inst_idx, n_prev_active_inst, n_bm)
            active_inst_idx_to_position_map = get_inst_idx_to_tensor_position_map(active_inst_idx_list)

            return input_mask, bs, active_src_seq,  active_utt_loc, active_inst_idx_to_position_map

        def beam_decode_step(input_mask, bs, inst_dec_beams, len_dec_seq, active_inst_idx_list, act_log, src_seq, \
                              utt_loc, inst_idx_to_position_map, n_bm):
            """ Decode and update beam status, and then return active beam idx """
            n_active_inst = len(inst_idx_to_position_map)
            ...
            logits, act_logits, _ = self.act_forward(dec_partial_seq, src_seq, bs, input_mask,  utt_loc)
        ...
        with torch.no_grad():
            ...
            for len_dec_seq in range(1, max_token_seq_len + 1):
                active_inst_idx_list = beam_decode_step(input_mask, bs, inst_dec_beams, len_dec_seq,
                                                        active_inst_idx_list,
                                                        act_logits, src_seq, utt_loc, inst_idx_to_position_map, n_bm)

                if not active_inst_idx_list:
                    break  # all instances have finished their path to <EOS>

                input_mask, bs, src_seq, utt_loc, inst_idx_to_position_map = collate_active_info(input_mask,
                                                                                        bs, src_seq, utt_loc,
